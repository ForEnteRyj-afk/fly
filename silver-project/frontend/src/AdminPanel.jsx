...
// --- Компонент: Карточка Узла (Визуальная часть) ---
const NodeCard = ({ node }) => (
...
// --- МОДАЛЬНОЕ ОКНО: РЕДАКТИРОВАНИЕ ЮЗЕРА ---
...
// --- МОДАЛЬНОЕ ОКНО: РЕДАКТИРОВАНИЕ КУРСОВ ---
const EditRatesModal = ({ rates, onClose, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in">
      <div className="w-full max-w-md bg-[#121214] border border-white/10 rounded-2xl p-6 shadow-2xl">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-xl font-bold text-white">Настройка курсов (к рублю ПМР)</h3>
          <button onClick={onClose}><X className="text-gray-500 hover:text-white" /></button>
        </div>
        <div className="space-y-4">
          <div>
...
export default function AdminPanel({ onClose }) {
  ...
  // Демо-данные (графики и узлы пока статические, но красивые)
  const MOCK_NODES = [
  ...
  useEffect(() => {
    fetchData();
    const interval = setInterval(fetchData, 5000); // Авто-обновление каждые 5 сек
  ...
  const fetchData = async () => {
    try {
        ...
        setRates(ratesData);
        setUsers(usersData);
    } catch (e) { console.error("Ошибка связи с сервером", e); }
  };

  // Сохранение курсов на сервер
  ...
  // Сохранение баланса юзера на сервер
  ...
  return (
    <div className="fixed inset-0 z-[100] min-h-screen bg-[#050507] text-gray-200 font-sans selection:bg-blue-500/30 flex overflow-hidden animate-in fade-in duration-300">
      
      {/* Sidebar */}
      <aside className="w-64 bg-[#0a0a0c] border-r border-white/5 flex flex-col">
        ...
        <nav className="flex-1 px-4 space-y-2 mt-4">
          ...
          <SidebarItem icon={LogOut} label="Выйти" active={false} onClick={onClose} />
        </nav>
      </aside>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        {/* Top Bar */}
        <header className="h-16 border-b border-white/5 bg-[#050507]/80 backdrop-blur-md flex items-center justify-between px-8 z-20">
        ...
        <div className="flex-1 overflow-y-auto p-8 scrollbar-hide">
           ...
           {/* === Вкладка 1: ДАШБОРД (Курсы и статистика) === */}
           {activeTab === 'dashboard' && (
             <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
                ...
                <div className="grid grid-cols-3 gap-6 h-96">
                   ...
                   <div className="p-6 rounded-2xl bg-white/[0.02] border border-white/[0.06] flex flex-col relative overflow-hidden group">
                      ...
                      <h3 className="font-bold text-white mb-4">Актуальные курсы</h3>
           ...
           {/* === Вкладка 3: ПОЛЬЗОВАТЕЛИ (Реальные данные) === */}
           {activeTab === 'users' && (
             <div className="animate-in fade-in slide-in-from-bottom-4">
                <div className="flex justify-between items-center mb-6">
